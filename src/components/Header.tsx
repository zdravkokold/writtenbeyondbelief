...
export function Header({ overlay = false }: { overlay?: boolean }) {
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-6 md:py-8">
        <Link to="/" className="flex min-w-0 items-center gap-3">
          <Logo className="h-14 w-14 shrink-0 md:h-16 md:w-16" />
          <span
            className={`font-display truncate text-lg tracking-wide md:text-2xl ${
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          className={`shrink-0 md:hidden ${overlay ? "text-white" : "text-foreground"}`}
        >
